...
\cite{Lv-extra-connectivity-and-extra-diagnosability}
和\cite{ A-New-Proof-for-Exact-Relationship}根据额外连通度直接得到扩展立方体的额外诊断度。
...
$n$-维扩展立方体$AQ_n$中的一对顶点$(u,v)$，如果满足条件$v=\bar{u}_n$，那么$(u,v)$被称为$AQ_n$中的一个补对。显然$AQ_n$中包含$2^{n-1}$个两两不相交的补对。
...
当$n \geq 4$时，设$x,y,s,t,u$是$AQ_n$中的5个顶点，如果$AQ_n$在$x,y,s,t,u$中包含一个补对，那么$|N_{AQ_n}(\{x,y,s,t,u\})| \geq 10n-39$。
\end{lem}
...
当$n \geq 4$时，设$x,y,s,t,u$是$AQ_n$中的5个顶点，如果$AQ_n$在$x,y,s,t,u$中包含两个补对，那么$|N_{AQ_n}(\{x,y,s,t,u\})| \geq 10n-35$。
\end{lem}
...
当$n \geq 4$时，设$x,y,s,t,u,v$是$AQ_n$中的6个顶点，如果$AQ_n$在$x,y,s,t,u,v$中包含三个补对，那么$|N_{AQ_n}(\{x,y,s,t,u,v\})| \geq 12n-46$。
\end{lem}

\begin{lem}
\textnormal{\upcite{t/t-Diagnosability-and-t/k-Diagnosability}}
\label{k-nodes}
...
\textnormal{(1)} $AQ_n-S$是连通的；

\textnormal{(2)} $AQ_n-S$有两个分支，其中一个是$K_1$，另一个包含$2^n-|S|-1$个顶点。
\end{thm}
...
\textnormal{(2)} $AQ_n-S$有两个连通分支，其中一个是$K_1$，另一个包含$2^n-|S|-1$个顶点；

\textnormal{(3)} $AQ_n-S$有两个连通分支，其中一个是$K_2$，另一个包含$2^n-|S|-2$个顶点；

\textnormal{(4)} $AQ_n-S$有三个连通分支，其中有两个是$K_1$，且第三个包含$2^n-|S|-2$个顶点。
\end{thm}
...
\upcite{A-New-Proof-for-Exact-Relationship}
...
\begin{lem}
\textnormal{\upcite{Conditional-Diagnosability-Measures-for-Large,On-self-diagnosable-multiprocessor}}
\label{MM*-distinguable}
对于图$G$中的两个不同的顶点子集$F_1$，$F_2 \subset V(G)$。$(F_1,F_2)$在\textnormal{MM}$^*$ 模型下是可区分的当且仅当下面三个条件中的一个成立（见图\ref{MM*}）：

\textnormal{(1)} 存在$x,z \in V(G)-(F_1 \cup F_2)$和$y \in (F_1-F_2) \cup (F_2-F_1)$，使得$xz,yz \in E(G)$；

\textnormal{(2)} 存在$x,y \in F_1-F_2$和$z \in V(G)-(F_1 \cup F_2)$，使得$xz,yz \in E(G)$；

\textnormal{(3)} 存在$x,y \in F_2-F_1$和$z \in V(G)-(F_1 \cup F_2)$，使得$xz,yz \in E(G)$。
\end{lem}
...
\begin{defn}
\textnormal{\upcite{A-New-Proof-for-Exact-Relationship}}
\label{MM*-distinguishable}
\textnormal{(1)} 在MM$^*$模型下，一个图$G$是$g$-额外，$t$-可诊断的，当且仅当对任意两个不同的$g$-额外集$F_1 \subseteq V(G)$，$F_2 \subseteq V(G)$，其中$|F_1| \leq t$，$|F_2| \leq t$，且$F_1$和$F_2$是可区分的。

\textnormal{(2)} $G$在MM$^*$下的$g$-额外诊断度，记为$t^g_M(G)$，是$G$在$g$-额外，$t$-可诊断下$t$的最大值。
\end{defn}
...
假设在$AQ_n-(F_1 \cup F_2)$中存在一个孤立顶点$w$。设$W$是$AQ_n-(F_1 \cup F_2)$中孤立顶点的集合，并且$H$是$AQ_n-(F_1 \cup F_2)$的导出子图。
...
因为$W$是$AQ_n-(F_1 \cup F_2)$中所有孤立顶点的集合，并且$H$是$V(AQ_n)-(F_1 \cup F_2)$ 中的导出子图，所以$N(V(H))\cap W=\emptyset$。

假设$N(V(H)) \cap (F_1 \Delta F_2)\neq \emptyset$，那么在$V(H)$中存在一个顶点$z$，使得
$z$在$F_1 \Delta F_2$中至少有1个邻居顶点$y$。因为在$V(H)$中的任意一个顶点在$AQ_n-(F_1 \cup F_2)$中都不是一个孤立的顶点，所以$z$在$V(H)$中也存在一个邻居顶点$x$。因此, $x,z \in V(G)-(F_1 \cup F_2)\neq \emptyset$，$y \in F_1 \Delta F_2$ 且$xz,yz \in E(AQ_n)$。根据引理\ref{MM*-distinguable}，$F_1$和$F_2$是可区分的，这与$(F_1,F_2)$在MM$^*$模型下是不可区分的相矛盾。因此$N(V(H)) \cap (F_1 \Delta F_2)= \emptyset$。
...
%$|H|>2^n - (18n-45) > 2$，
$|H|>2^n - 2(6n-15) > 2$，
且$|V(Y)|>2$。 这意味着$AQ_n-F_1 \cap F_2$中包含两个顶点个数大于2的连通分支，与定理\ref{6n-18<=}矛盾。因此，$AQ_n-(F_1 \cup F_2)$不包含孤立顶点。
...
当$n \geq 4$时，设$x,y,s,t,u,v$是$AQ_n$中的6个不同顶点，如果$AQ_n$在$x,y,s,t,u,v$中包含2个补对，那么$|N_{AQ_n}(\{x,y,s,t,u,v\})| \geq 12n-48$。
\end{lem}

\begin{proof}
设$(x,y)$，$(s,t)$是$AQ_n=L \bigodot R$中的2个补对，其中$L \cong AQ^0_{n-1}$，$R \cong AQ^1_{n-1}$。不失一般性，我们设$x,s \in V(L)$，$y,t\in V(R)$。我们有下面几种情况：

\textbf{情形1}：$u,v \in L$或$u,v \in R$。不失一般性，我们假设$u,v \in L$，根据引理\ref{2nodes}和引理\ref{4nodes}，我们有$|N_L(\{x,s,u,m\})| \geq 8(n-1)-28=8n-36$且$|N_L(\{y,t\})|=4(n-1)-8$。因此
\begin{align*}
...
当$n \geq 4$时，设$x,y,s,t,u,v,m$是$AQ_n$中的7个顶点，如果$AQ_n$在$x,y,s,t,u,v,m$中包含$3$ 个补对，那么$|N_{AQ_n}(\{x,y,s,t,u,v,m\})| \geq 14n-59$。
\end{lem}
...
当$n \geq 4$时，设$x,y,s,t,u,v,m,n$是$AQ_n$中的8个顶点，如果$AQ_n$在$x,y,s,t,u,v,\\m,n$中包含4 个补对，则$|N_{AQ_n}(\{x,y,s,t,u,v,m,n\})| \geq 16n-72$。
\end{lem}
...
$L-S_L$中的一个顶点$u$被称为是孤单顶点，
如果$S_R$包含$u$的两个在$R$中的邻居顶点。否则称$u$为非孤单顶点。
...
%
\textbf{情形2.} $L-S_L$和$R-S_R$都是连通的。设$C_0=L-S_L$且$C_1=R-S_R$。
在$L$和$R$的$2^n$条边中，最多有$2|S|$条边与$S$中的顶点关联。因为$2^n \geq 2(6n-15)$，所以$C_0$和$C_1$连通。于是，$AQ_n-S$是连通的，且其顶点个数为$2^n-6n+15$。
...
不失一般性，假设顶点$v$存在一个邻居顶点，使得该邻居顶点属于$L-S_L-\{x,y,s,t,u,v\}$。另外，根据引理\ref{5-vertices-1-complement-pairs}和引理\ref{5-vertices-2-complement-pairs}，对于$n \geq 8$，有$|N_L(\{x,y,s,t,u\})| \geq 10(n-1)-39=10n-49>6n-15-6$，则在$x,y,s,t,u$中至少存在一个顶点，使得该顶点在$L-S_L-\{x,y,s,t,u\}$中存在一个邻居顶点。于是$AQ_n-S$中存在一个连通分支，其顶点个数为$2^n-6n+10+1=2^n-6n+11$。
...
\begin{lem}
\label{6n-15}
设$S$是$AQ_n$的一个顶点子集，其中$|S| \leq 6n-15$, $n \geq 8$。那么$AQ_n-S$满足以下$7$个条件之一:
...
\textnormal{(4)} $AQ_n-S$有两个连通分支，其中一个包含$3$个顶点，另一个包含$2^n-|S|-3$个顶点；

\textnormal{(5)} $AQ_n-S$有三个连通分支，其中有两个是$K_1$，且第三个包含$2^n-|S|-2$个顶点。

\textnormal{(6)} $AQ_n-S$有三个连通分支，其中一个是$K_1$，另一个是$K_2$，且第三个包含$2^n-|S|-3$个顶点。

\textnormal{(7)} $AQ_n-S$有四个连通分支，其中有三个是$K_1$，且第三个包含$2^n-|S|-3$个顶点。
\end{lem}

根据引理\ref{6n-15}，可以得到下面的定理：

\begin{lem}
\label{two-components}

设$F$是$AQ_n$的一个$2$-额外集，其中$|F| \leq 6n-15$, $n \geq 8$。那么$AQ_n-S$满足以下$2$ 个条件之一:

\textnormal{(1)} $AQ_n-F$是连通的；

\textnormal{(2)} $AQ_n-F$包含两个连通分支，其中一个是包含顶点个数为$3$的连通分支，另一个是包含顶点个数为$2^n-|F|-3$的连通分支。
\end{lem}

\begin{defn}
\upcite{Reliability-analysis-of-data-center-networks}
一个系统$G=(V,E)$和一个由故障顶点集在$G$上产生的症状$\sigma$。设$u \subseteq V$，
...
\begin{lem}
\textnormal{\upcite{Reliability-analysis-of-data-center-networks}}
\label{vertex-judge-principle}
给定一个最多有$t$个故障顶点的系统$G=(V,E)$和一个在\textnormal{MM}$^*$模型下的$G$上的症状$\sigma$。

\textnormal{(1)} 对于任意两个顶点$u,v \in V$，$(u,v) \in E$，其中$v \in C_0(u)$，$u \in C_0(v)$，那么$u$和$v$有相同的状态（故障或无故障）。

\textnormal{(2)} 对于任意一个连通分支$R \subseteq T_0(G)$，那么在$R$中的所有顶点，要么都是故障顶点，要么都是无故障顶点。

\textnormal{(3)} 如果$T_0(G)$的连通分支$R$的顶点个数$|V(R)| \geq t+1$，那么$R$中的所有顶点都是无故障的。
\end{lem}


在参考文献\cite{Reliability-analysis-of-data-center-networks}中的诊断算法后，我们给出了扩展立方体在MM$^*$模型下$AQ_n$的$2$-额外，$(6n-15)$-可诊断算法。
...
并且这个算法可以正确地诊断出$AQ_n$中的所有的故障顶点。
%对于一个扩展立方体，我们使用邻接矩阵去存储扩展立方体的结构。
...
%\begin{algorithmic}[1]
%    \Require 一个$n$-维的扩展立方体$AQ_n$；一个$2$-额外故障顶点集$F$生成的症状$\sigma$，其中$|F| \leq 6n-15$；两个无故障的顶点$u,v \in V(AQ_n)$，其中$(u,v)\in E(AQ_n)$。
%    \Ensure  一个无故障顶点集合$T$。
%   \State $T \leftarrow \{u,v\}$;
%   \Function{\texttt{DFS-MM$^*$}}{$AQ_n,\sigma,u,v$}
%   \For{$j \in N_{AQ_n}(v)- T$}
%        \If{$\sigma(u,j)_v=0$}
%            \State $T \leftarrow T \cup \{j\}$;
%            \State \texttt{AQ-DFS-MM$^*$}($AQ_n,\sigma,v,j$);
%        \EndIf
%   \EndFor\\
%   \Return $T$;
%   \EndFunction
...
\begin{algorithmic}[1]
    \Require 一个$n$-维的扩展立方体$AQ_n$；一个$2$-额外故障顶点集$F'$生成的症状$\sigma$，其中$|F'| \leq 6n-15$；两个无故障的顶点$u,v \in V(AQ_n)$，其中$(u,v)\in E(AQ_n)$。
    \Ensure  一个无故障顶点集合$T$，一个故障顶点集合$F$。

   \Function{\texttt{AQ-DFS-MM$^*$}}{$AQ_n,\sigma,u,v$}
   \State $T \leftarrow \emptyset,F \leftarrow \emptyset$;
   \For{$j \in N_{AQ_n}(v)- T-F$}
        \If{$\sigma(u,j)_v=0$}
            \State $T \leftarrow T \cup \{j\}$;
            \State $T',F' \leftarrow$ \texttt{AQ-DFS-MM$^*$}($AQ_n,\sigma,v,j$);
            \State $T \leftarrow T \cup T'$;
            \State $F \leftarrow F \cup F'$;
        \Else
            \State $F \leftarrow F \cup \{j\}$;
        \EndIf
   \EndFor
   \State \Return $T,F$;
   \EndFunction
...
%\ref{vertex-judge-principle}来得到$T_0(G)$。
因为算法AQ-DFS-MM$^*$是通过递归的深度优先算法来实现，所以算法AQ-DFS-MM$^*$的时间复杂度为$O(N^2)$，其中$N$为扩展立方体的顶点个数。
...
%    \Require 一个$n$-维的扩展立方体$AQ_n$；一个$2$-额外故障顶点集$F$生成的症状$\sigma$，其中$|F| \leq 6n-15$。
%    %\Ensure  集合$T,F',H$。
%    \Ensure  一个故障顶点集合$F'$。
%    \Function{\texttt{AQ-EXTRA-DIAG}}{$AQ_n,\sigma$}
%    \State $T \leftarrow \emptyset, F' \leftarrow \emptyset, R\leftarrow V(AQ_n)$;
%    \State 在$R$中选择$2$个无故障顶点$u,v$，其中$(u,v) \in E(AQ_n)$;
%    \State $T \leftarrow \texttt{AQ-DFS-MM*}(AQ_n,\sigma,u,v)$;
%    \If{$|T| \leq 3$}
%        %\State $F' \leftarrow N_{AQ_n}(T)$;
%        \State 重新选择$2$个无故障顶点$m,n$，其中$(m,n) \in E(AQ_n)$;
%        %\State $T \leftarrow \emptyset$;
%        \State $T \leftarrow T \cup \texttt{AQ-DFS-MM$^*$}(AQ_n,\sigma,m,n)$;
%        \State $F' \leftarrow R-T$;\\
%        \quad\quad\quad\Return $F'$;
%    \EndIf
%    \State $R \leftarrow R-T$;
%    \For{对于顶点$x \in N_{AQ_n}(T) \cap R,$，$y,r \in T$，其中$(x,r) \in E(AQ_n)$且$(y,r)\in E(AQ_n)$}
%            \If{$\sigma(x,y)_r=1$}
%                \State $F'=F' \cup \{x\}$;
%            \EndIf
%        \EndFor
...
\begin{algorithmic}[1]
    ...
    \State $T \leftarrow \emptyset, F \leftarrow \emptyset, R\leftarrow V(AQ_n)$;
    %\State 在$R$中选择$2$个无故障顶点$u,v$，其中$(u,v) \in E(AQ_n)$;
%    \State $T,F \leftarrow \texttt{AQ-DFS-MM*}(AQ_n,\sigma,u,v)$;
    \While{$|R|>0$}
        \State 在$R$中选择$2$个无故障顶点$u,v$，其中$(u,v) \in E(AQ_n)$;
        ...
        \State $T \leftarrow T \cup T'$;
        \State $F \leftarrow F \cup F'$;
        \State $R \leftarrow R - T' - F'$;
        \If{$|F|=6n-15$ or $R=\emptyset$}
            %\quad\quad\quad\quad\Return $F'$;
            \State break;
        %\ElsIf{$R=\emptyset$}
%            \State execution;
        \EndIf
...
%得到的是小的无故障连通分支$C$且$|V(C)|=3$，
%调用算法AQ-DFS-MM$^*$可以将$C$的所有无故障顶点都诊断出来，
%然后再次调用算法AQ-DFS-MM$^*$，这次可以得到另一个更大的无故障连通分支$H$且$|V(H)|=2^n-|F|-3$。
%这样我们就可以得到所有的无故障顶点，因此剩下的就都是故障顶点；
%（2）我们通过调用算法AQ-DFS-MM$^*$得到一个连通分支$H$且$|V(H)|>3$，
...
%
在算法执行过程中，因为无故障顶点的数量相对于故障顶点数的数量要多得多，所以很容易选取到两个无故障顶点。根据这两个无故障顶点，通过调用算法AQ-DFS-MM$^*$可以很容易找到一个无故障分支。因为故障顶点数最多为$6n-15$，如果已经找到的故障顶点数等于$6n-15$，则剩下的未确定状态的顶点都是无故障的。此外，如果无故障顶点个数加上故障顶点个数等于扩展立方体的总个数，则此时也完成了所有故障顶点的查找。因为算法以引理\ref{two-components}为基础，所以算法AQ-EXTRA-DIAG中的循环最多执行两次。

因为算法AQ-EXTRA-DIAG的时间复杂度主要消耗在调用算法AQ-DFS-MM$^*$上，且算法AQ-EXTRA-DIAG的循环最多执行两次。所以算法AQ-EXTRA-DIAG的时间复杂度是$O(N^2)$，其中$N$ 为扩展立方体的顶点个数。
...
内存8GB的计算机。我们将通过这台计算机模拟算法并给出实验结果。
...
\begin{itemize}
    \item $n=8$
    \item $f=33$
    \item $F=[35, 209, 45, 38, 81, 100, 241, 175, 109, 145, 238, 178, 18, 24, 52, 69, 242,\\ 201, 22, 82, 53, 203, 64, 0, 114, 55, 62, 67, 57, 72, 78, 206, 23]$
    %\item $SF=[0, 1, 2]$
    \item $avg_t=0.01096$
\end{itemize}
...
4. 第四组测试数据和结果$E(11,47)$

\begin{itemize}
    \item $n=11$
    \item $f=47$
    \item $F=[1416, 280, 1759, 768, 138, 1326, 934, 407, 1873, 339, 799, 1506, 625, 375, 657, 993,\\ 441, 752, 439, 788, 1116, 1595, 502, 1524, 1424, 1010, 1883, 205, 1073, 168, 52, 281,\\ 1856, 263, 1594, 409, 365, 1293, 1528, 927, 300, 349, 883, 1226, 1036, 687, 343]$
...
5. 第五组测试数据和结果$E(17,55)$

\begin{itemize}
    \item $n=17$
    \item $f=55$
    \item $F=[1942, 3465, 2618, 1776, 8, 3767, 1637, 1143, 1544, 1436, 574, 1009,\\ 3935, 3359, 3883, 17, 2948, 1258, 1428, 137, 2305, 435, 2585, 313, 3356, \\ 440, 2030, 696, 369, 2779, 207, 2781, 2299, 3711, 1540, 2586, 3150, 3136,\\ 3944, 2732, 2063, 763, 278, 2972, 2713, 128, 835, 619, 3600, 3318, 3847, \\ 25, 392, 380, 1575]$
...
从实验结果可以看出，算法AQ-EXTRA-DIAG可以有效诊断出所有的故障顶点。
%而且图~\ref{AQ-EXTRA-DIAG-time-diagram}显示，算法的时间变化趋势也和我们在前面分析的算法时间复杂度类似。
