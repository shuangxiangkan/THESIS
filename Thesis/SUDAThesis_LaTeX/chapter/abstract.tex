...
随着我国在超级计算机方面取得的一系列丰硕成果，高性能计算受到了越来越多的关注。由于巨大的数据处理和数值计算能力，高性能计算被广泛应用于国防建设和国民经济，是衡量一个国家综合国力的重要标志之一。互连网络作为高性能计算的重要组成部分，其性能的好坏对于高性能计算至关重要。

决定互连网络性能的因素有很多，容错性能及可靠性是衡量一个互连网络性能的重要参考标准。一个具有好的容错性质的互连网络，即使网络中的一些服务器发生故障，也不会对整个网络的性能产生太大的影响。连通度是衡量一个网络的重要指标之一。但是传统的连通度往往假设网络中的顶点发生故障时，其对周围的顶点没有影响。然而在实际的运行环境中，当一个顶点发生故障时，其周围的顶点发生故障的概率也大大增加。为了弥补这种缺陷，结构连通度和子结构连通度从更实际的角度考察了互连网络发生故障的情形，这两种连通度从故障结构而不是故障顶点的角度来考虑网络的容错能力。另一方面，当网络中的顶点发生故障时，如何将这些故障顶点准确地诊断出来，并将故障顶点替换为无故障顶点以保证互连网络的正常运行，也是互连网络需要重点考虑的方面。一个互连网络所能确定的最大的故障顶点数叫做这个系统的诊断度。诊断度同样也是衡量互连网络容错性能的一个关键指标。



扩展立方体，是著名互连网络超立方体拓扑结构的一种重要变体。扩展立方体不仅保留了超立方体的许多优秀的性质，同时拥有超立方体和它的其他变体所不具备的优点。比如，扩展立方体的连通度是$2n-1$，几乎是超立方体的两倍。这也意味着扩展立方体的容错性能在一些方面远胜过超立方体。

本文主要研究了扩展立方体的容错方面的性质，主要包括结构连通度，子结构连通度以及额外诊断度。同时提出了容错路由算法和诊断算法。具体成果如下：

（1）本文从故障结构而非故障顶点的角度研究了扩展立方体的结构连通度和子结构连通度，同时提出了在$K_{1,6}$子结构连通度下的容错路由算法，且通过模拟实验验证了算法的有效性和准确性。
...
\keywords{高性能计算，扩展立方体，结构连通度，子结构连通度，额外连通度，额外诊断度，容错路由算法，诊断算法}
